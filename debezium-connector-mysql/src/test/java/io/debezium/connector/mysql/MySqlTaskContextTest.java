...
public class MySqlTaskContextTest {
    ...
    @Test
    public void shouldFilterSomeMariaDbUsingCustomFilter() throws Exception {
        config = simpleConfig().with("internal.database.history.ddl.filter", "DELETE\\s*FROM \\s*.*,DROP TEMPORARY TABLE IF EXISTS .+ /\\* generated by server \\*/,INSERT INTO mysql.rds_heartbeat2\\(.*\\) values \\(.*\\) ON DUPLICATE KEY UPDATE value = .*,DELETE FROM mysql.rds_sysinfo.*,INSERT INTO mysql.rds_sysinfo\\(.*\\) values \\(.*\\),INSERT INTO mysql.rds_monitor\\(.*\\) values \\(.*\\) ON DUPLICATE KEY UPDATE value = .*,INSERT INTO mysql.rds_monitor\\(.*\\) values \\(.*\\),DELETE FROM mysql.rds_monitor.*,FLUSH RELAY LOGS.*,flush relay logs.*,SAVEPOINT .*").build();
        context = new MySqlTaskContext(config, new Filters.Builder(config).build(), false, null);
        assertThat(context.ddlFilter().test("DELETE FROM `trubendorffer`.`behandelingverzamelingggz`")).isTrue();
    }
